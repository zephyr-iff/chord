...
describe('Chord Matrix', () => {
  ...
  it('Should correctly initalize the cells when increasing the number of frets', () => {
    ...
    matrix.rows.forEach((row) => {
      expect(row).toEqual([{ state: CellState.INACTIVE }, { state: CellState.INACTIVE }, { state: CellState.INACTIVE }])
    })
  })
  ...
  it('Should correctly "cut off" a barre chord when reducing the number of strings', () => {
    ...
    // then
    expect(matrix.rows[0].map(({ state }) => state)).toEqual([CellState.LEFT, CellState.RIGHT])
  })
  ...
  it('Should correctly compute empty strings after increasing the number of strings', () => {
    ...
    // then
    expect(matrix.getEmptyStringStates()).toEqual([EmptyStringState.O, EmptyStringState.O, EmptyStringState.O, EmptyStringState.O])
  })
  ...
  it('Should correctly keep the state when increasing the number of strings', () => {
    ...
    // then
    expect(matrix.rows[0].map(({ state }) => state)).toEqual([CellState.LEFT, CellState.MIDDLE, CellState.RIGHT, CellState.INACTIVE])
    expect(matrix.rows[2].map(({ state }) => state)).toEqual([CellState.LEFT, CellState.MIDDLE, CellState.RIGHT, CellState.INACTIVE])
  })
  ...
  it('Should correctly set the state of connected cells', () => {
    ...
    // then
    expect(matrix.rows[1].map(({ state }) => state)).toEqual([CellState.LEFT, CellState.MIDDLE, CellState.RIGHT])
  })
  ...
  it('Should correctly set the state of highlighted cells', () => {
    ...
    // then
    expect(matrix.rows[1].map(({ state }) => state)).toEqual([CellState.LEFT_HL, CellState.MIDDLE_HL, CellState.RIGHT_HL])
  })
  ...
  it('Should un-highlight cells on the same fret when highlighting other cells on the same fret', () => {
    ...
    // then
    expect(matrix.rows[1].map(({ state }) => state)).toEqual([CellState.INACTIVE, CellState.LEFT_HL, CellState.RIGHT_HL])
  })
  ...
  it('Should remove the connected cells when a string is toggled on the same fret', () => {
    ...
    // then
    expect(matrix.rows[1].map(({ state }) => state)).toEqual([CellState.INACTIVE, CellState.ACTIVE, CellState.INACTIVE])
  })
  ...
  it('Should correctly convert to and from vexchord', () => {
    ...
    // then
    expect(matrix.toVexchord()).toEqual(ChordMatrix.fromChart({ chord: matrix.toVexchord(), settings }).toVexchord())
  })
  ...
  it('Should correctly set colors', () => {
    ...
    // then
    expect(matrix.toVexchord().fingers).toContainEqual([2, 2, { color: 'red' }])
  })
  ...
  it('Should allow setting a finger next to a barre chord', () => {
    ...
    // then
    expect(matrix.rows[0].map(({ state }) => state)).toEqual([CellState.LEFT, CellState.RIGHT, CellState.ACTIVE])
  })
  ...
  it('Should remove barre chords that are "cut off" from the left', () => {
    ...
    // then
    expect(matrix.rows[0].map(({ state }) => state)).toEqual([CellState.INACTIVE, CellState.LEFT, CellState.RIGHT])
  })
  ...
  it('Should clear all state from the barre chord when it is removed', () => {
    ...
    // then
    expect(matrix.rows[0]).toEqual([{ state: CellState.ACTIVE }, { state: CellState.INACTIVE }, { state: CellState.INACTIVE }])
  })
